...
* Match the precedent set by ADR-0010 + the Atlassian tool — auth flows through the Connections tab, no extra runtime deps, the same proxy / CA bundle plumbing as everything else.
...
* **Native REST tools** — ship `lib/tools/github.ts` with a small set of issue/PR tools modeled on `atlassian.ts`. Auth: PAT from env or the built-in integrations section under Connections.
...
Initial surface (7 tools): `github_search_issues`, `github_get_issue`, `github_create_issue`, `github_add_comment`, `github_list_pulls`, `github_get_pull`, `github_get_repo`. Auth: `GITHUB_TOKEN` or `GH_TOKEN` env, falling back to the Connections tab record.
...
* **Good** — The gear-menu Connections tab handles credential entry + a real `GET /user` smoke test, matching the UX pattern users already know from Atlassian/Gmail/Outlook.
...
* Good, because credentials live where every other built-in integration credential lives (encrypted at rest under `JARELA_DB_DIR`).
