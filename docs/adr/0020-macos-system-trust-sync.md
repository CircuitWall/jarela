...
* **One-click setup on managed macOS.** A corporate-managed Mac already
...
* **Option D:** Cross-platform via `mac-ca` / `win-ca` / a Linux
  ...
  pain (Visa managed Macs); Linux + Windows can follow with their own
...
* Disk footprint: one PEM file in `JARELA_DB_DIR` (~200 KB on a Visa
...
* Bad, every Visa managed Mac re-discovers the same trap on first
  install.
