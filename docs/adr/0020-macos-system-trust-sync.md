...
macOS hosts behind a TLS-intercepting corporate proxy:
...
* **One-click setup on macOS.** A macOS host whose keychain is already
  populated with the corporate roots has every CA the enterprise wants
  trusted. Asking the user to paste
...
* **Option D:** Cross-platform via `mac-ca` / `win-ca` / a Linux
  ...
  pain (macOS hosts on corporate networks); Linux + Windows can follow with their own
...
* Disk footprint: one PEM file in `JARELA_DB_DIR` (~200 KB on a typical
  corporate keychain). Regenerated on every `mode=system` apply, so it stays
...
* Good, because the OS keychain is already the source of truth for the
  host's trusted roots (MDM or manual installs both land there).
...
* Bad, every macOS host on a corporate network re-discovers the same trap on first
  install.
