  ...
  return (
    <div className="flex flex-col h-full">
      <div className="border-b border-border px-4 py-3 flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-zinc-100 mr-auto">MCP Servers</h2>
        <button
            ...
            <div key={s.name} className="flex items-center gap-3 py-2.5 border-b border-border/60 group">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <button
                    onClick={() => toggle(s)}
                    className={s.enabled ? "text-emerald-400" : "text-zinc-600 hover:text-zinc-400"}
                    title={s.enabled ? "Disable" : "Enable"}
                  >
                    {s.enabled ? <CheckCircle2 size={13} /> : <AlertCircle size={13} />}
                  </button>
                  <span className="text-sm font-medium text-zinc-100">{s.name}</span>
                  <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded border bg-zinc-800 text-zinc-400 border-zinc-700">
                    {s.transport}
                  </span>
                </div>
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button onClick={() => setEditing({ mode: "form", existing: s })} className="p-1 text-zinc-400 hover:text-zinc-100 transition-colors" title="Edit">
                  <span className="text-xs">edit</span>
                </button>
                <button onClick={() => remove(s.name)} className="p-1 text-zinc-400 hover:text-red-400 transition-colors" title="Delete">
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
    ...
    <div className="absolute inset-0 bg-black/60 z-30 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface-2 border border-border rounded-xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="px-4 py-3 border-b border-border flex items-center gap-1">
          {onBack && (
            <button onClick={onBack} className="p-1 text-zinc-400 hover:text-zinc-100" title="Back to picker">
              <ChevronLeft size={14} />
            </button>
          )}
          <h3 className="text-sm font-semibold text-zinc-100 mr-auto">
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          {registryEntry && (
            <div className="px-3 py-2 rounded bg-surface-3/40 border border-border text-[11px] text-zinc-400">
          ...
          {registryEntry?.variables?.length ? (
            <div className="space-y-2">
              <p className="text-xs text-zinc-400 font-medium">Configure</p>
              {registryEntry.variables.map((v) => (
                <label key={v.key} className="block text-xs text-zinc-400">
                  {v.label}
                  <input
          ...
          <label className="block text-xs text-zinc-400">
            ...
            <input
              ...
              disabled={!!server}
              className="mt-1 w-full px-2 py-1.5 text-sm rounded border border-border bg-surface-3 text-zinc-100 disabled:opacity-50"
              placeholder="filesystem"
            />
          </label>

          <div>
            <p className="text-xs text-zinc-400 mb-1">Transport</p>
          ...
          <label className="block text-xs text-zinc-400">
            Spec (JSON)
            <p className="mt-0.5 mb-1 text-[11px] text-zinc-500">
              {transport === "stdio"
            ...
            <textarea
              ...
              rows={8}
              className="w-full px-2 py-1.5 text-xs font-mono rounded border border-border bg-surface-3 text-zinc-100"
            />
          </label>
        ...
        <div className="px-4 py-3 border-t border-border flex justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1.5 text-xs text-zinc-400 hover:text-zinc-100">
            Cancel
          </button>
          <button
            onClick={save}
    ...
    <div className="absolute inset-0 bg-black/60 z-30 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="px-4 py-3 border-b border-border flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-zinc-100 mr-auto">Add MCP server</h3>
          <button onClick={onClose} className="p-1 text-zinc-400 hover:text-zinc-100">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="px-4 py-2 border-b border-border">
          <input
            ...
            className="w-full px-2 py-1.5 text-sm rounded border border-border bg-surface-3 text-zinc-100"
            autoFocus
          />
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-2">
          {loading && <p className="text-zinc-500 text-sm py-6 text-center">Loading…</p>}
          {!loading && grouped.length === 0 && (
            <p className="text-zinc-500 text-sm py-6 text-center">No matches.</p>
          )}
          {grouped.map(([cat, items]) => (
            <div key={cat} className="mb-3">
              <p className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1.5 mt-1">{cat}</p>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg border transition-colors group ${
                        installed
                          ...
                          : "bg-surface-3/60 hover:bg-surface-3 border-border hover:border-zinc-600"
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className={`text-sm font-medium ${installed ? "text-zinc-500" : "text-zinc-100"}`}>{e.name}</span>
                        <span className="text-[9px] uppercase tracking-wider px-1 py-px rounded border border-border text-zinc-500">
                          {e.source}
                        </span>
                        {installed && (
                      ...
                      <p className={`text-xs ${installed ? "text-zinc-600" : "text-zinc-400"}`}>{e.description}</p>
                    </button>
                  );
                })}
              </div>
