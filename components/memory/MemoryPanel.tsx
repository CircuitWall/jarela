  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-2 space-y-2 border-b border-border">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input className="w-full bg-surface-3 text-zinc-100 text-sm rounded px-2 py-1.5 pl-7 border border-border focus:outline-none focus:ring-1 focus:ring-accent"
              placeholder="Search…" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />
          </div>
          <select className="bg-surface-3 text-zinc-100 text-sm rounded px-2 py-1.5 border border-border focus:outline-none focus:ring-1 focus:ring-accent"
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2">
        {loading && items.length === 0 && <p className="text-zinc-500 text-sm text-center py-8">Loading…</p>}
        {!loading && items.length === 0 && <p className="text-zinc-500 text-sm text-center py-8">No memory items yet</p>}
        {items.map((item) => (
          <div key={`${item.namespace}/${item.key}`} className="flex items-start gap-2 py-2.5 border-b border-border/60 group">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-zinc-400 truncate">{JSON.stringify(item.value)}</p>
            </div>
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              <button onClick={() => setEditing(item)} className="p-1 text-zinc-400 hover:text-zinc-100 transition-colors" title="Edit"><Pencil size={13} /></button>
              <button onClick={() => remove(item.namespace, item.key)} className="p-1 text-zinc-400 hover:text-red-400 transition-colors" title="Delete"><Trash2 size={13} /></button>
            </div>
          </div>
        ))}
      </div>
