// Proxy configuration store (ADR-0009).
//
...
// so the on-disk DB cannot reveal the proxy credential.
//
// Two read shapes:
...
export function saveProxyConfig(input: ProxyConfigInput): ProxyConfigStatus | { error: string } {
  ...
  const db = getDb();
  db.prepare(
    `INSERT INTO proxy_config (id, mode, host, port, username, password, no_proxy, updated_at)
     VALUES (1, ?, ?, ?, ?, ?, ?, ?)
...
function readRow(): Row | null {
  const db = getDb();
  const r = db.prepare("SELECT mode, host, port, username, password, no_proxy, updated_at FROM proxy_config WHERE id = 1").get() as
    | Row
    | undefined;
  return r ?? null;
}
