...
export interface SystemPromptContext {
  ...
  /**
   * ADR-0061 — channel that delivered the active user-role turn. Drives the
   * "active turn channel" cue so the LLM knows the latest message arrived
   * via scheduler / watcher / bridge framing rather than plain chat, and
   * shouldn't anchor on prior plain-text turns when the inbound carries
   * envelope chrome. `null` / undefined = ordinary chat (no cue rendered).
   */
  inboundCategory?: string | null;
  /**
   * ADR-0061 — paired with `inboundCategory`. When true, the cue tells the
   * agent it may reply with the `NO_REPLY` sentinel for non-material work
   * (scheduler/watcher silent firings, bridge observer-mode routes).
   */
  inboundSilent?: boolean;
}

export function buildSystemPrompt(ctx: SystemPromptContext): string {
  const { agentCfg, trimmedMessage, budget, recallCtx, warmSummaryCtx, factsCtx, experienceMode, delegateRosterLines, taskGoal, inboundCategory, inboundSilent } = ctx;
  ...
  const parts: (string | null | undefined)[] = [
    ...
    buildInboundChannelContext(inboundCategory ?? null, inboundSilent === true),
...
// ADR-0061 — surface the channel that delivered the active user-role turn so
// the LLM correctly orients to a scheduler / watcher / bridge inbound rather
// than treating the envelope as routing chrome and anchoring on a prior plain
// chat turn. Returns "" for `null` / unknown categories so plain chat keeps
// behaving exactly as before. Exported for direct unit coverage.
export function buildInboundChannelContext(category: string | null, silent: boolean): string {
  if (!category) return "";
  switch (category) {
    case "bridge":
      return [
        "--- Active turn channel ---",
        "Your most recent user-role message was delivered via a **bridge** channel (WhatsApp, Telegram, Slack, etc.).",
        "The body begins with a metadata envelope — `[bridge:…]` `[chat_id:…]` `[chat_name:…]` `[chat_type:…]` `[message_role:…]` `[sender_id:…]` `[sender_name:…]` (group chats also include `[group_name:…]` / `[participant_*:…]`) — followed by a blank line, then the raw sender text.",
        "The raw text after the envelope IS the active request — respond to it, not to the envelope. Use `[message_role]` (`user` / `counterpart` / `agent`) to decide who you are addressing in any outbound chat reply.",
      ].join("\n");
    case "watcher":
      return [
        "--- Active turn channel ---",
        "Your most recent user-role message was generated by a **watcher** that polled a tool and detected a change.",
        "The body lists the watcher label, the tool called, its args, a previous→current diff, and a directive at the end. The diff is the evidence; the directive is the request.",
        "This is the active turn — act on the directive now using the diff as evidence. Do not treat the diff as background context to be acknowledged.",
      ].join("\n");
    case "scheduled_task": {
      const lines = [
        "--- Active turn channel ---",
        "Your most recent user-role message was fired by a **scheduled task** (cron-like). The body is the user-authored prompt for that task. Execute it now as the active turn, exactly as if the user had typed it themselves.",
      ];
      if (silent) {
        lines.push(
          "This task is configured **silent**: only surface a reply when there is something material the user needs to see right now (state changes, anomalies, action items). If after executing the task there is genuinely nothing material to report, reply with the single token `NO_REPLY` and nothing else. Do NOT use `NO_REPLY` to skip work — run the task first, decide afterward.",
        );
      }
      return lines.join("\n");
    }
    default:
      // Unknown / forward-compatible categories (e.g. `delegation`,
      // `synthetic`) — no specialised cue. Fall through to default chat
      // framing so adding a new category doesn't silently confuse the LLM.
      return "";
  }
}
