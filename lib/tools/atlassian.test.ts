...
import type { JiraFieldDef } from "./atlassian";

// Isolated SQLite per test process; the integrations store opens the DB on
// first import. Set env vars before importing the module under test so
// resolveAuth() picks up the test creds and never touches the saved store.
const tmpRoot = mkdtempSync(join(tmpdir(), "jarela-test-atlassian-"));
