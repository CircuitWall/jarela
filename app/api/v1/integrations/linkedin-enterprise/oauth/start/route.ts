...
export async function POST(req: NextRequest) {
  ...
  try {
    const redirectUri = `${req.nextUrl.origin}/api/v1/integrations/linkedin-enterprise/oauth/callback`;
    const result = createFlow("enterprise", { clientId: parsed.data.client_id, clientSecret: parsed.data.client_secret, credentialId: parsed.data.credential_id, scopes: parsed.data.scopes, redirectUri });
    return NextResponse.json({ authorize_url: buildAuthorizeUrl("enterprise", result.flow, result.state), state: result.state, redirect_uri: redirectUri });
